...
class Lista:
    ...
    def __init__(self, max):
        self.__items = np.empty(max, dtype=Provincia)
        self.__max = max
        self.__numItems = 0
    ...
    def insertar(self, item): 
        if self.__numItems <= self.__max:
            if self.vacia():
                ...
                i = 1
                while i <= self.__numItems and self.__items[i].getNombre() != item.getNombre():
                    i += 1
                if self.__items[i].getNombre() == item.getNombre():
                    self.__items[i].acumular(item.getSuperficie())
                for pos in range(self.__numItems, i - 1, - 1):
    ...
    def recorrer(self):
        for i in range(self.__numItems):
            print(self.__items[i].getSuperficie())
